...
Процедура СоздатьРаспаковщик()
	...
	Если Распаковщик = Неопределено Тогда
		...
		Распаковщик.ОсталосьОпераций(ОсталосьОпераций());
...
Функция НомерОчереди()
	...
	Если Количество > 1 Тогда
		Для Сч = 0 По Количество - 1 Цикл
			Если Лев(Строка(АргументыКоманднойСтроки[Сч]),2) = "-n" Тогда
				Возврат Сред(Строка(АргументыКоманднойСтроки[Сч]),3)*1;
			КонецЕсли;	
		КонецЦикла;
	КонецЕсли;
...
Функция ОсталосьОпераций()
	Количество = АргументыКоманднойСтроки.Количество();
	Если Количество > 1 Тогда
		Для Сч = 0 По Количество - 1 Цикл
			Если Лев(Строка(АргументыКоманднойСтроки[Сч]),6) = "-count" Тогда
				Возврат Сред(Строка(АргументыКоманднойСтроки[Сч]),7)*1;
			КонецЕсли;	
		КонецЦикла;
	КонецЕсли;
