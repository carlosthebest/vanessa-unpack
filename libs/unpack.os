...
Функция СформироватьКомментарийСУчетомПромежуточныхВерсий(Знач ТаблицаИсторииХранилища, Знач ТекущаяВерсия, Знач СледующаяВерсия)
	...
	Если СледующаяВерсия - ТекущаяВерсия = 1 Тогда
...
Функция ДобавитьВыгруженныеФайлыВАрхив(СтруктураПараметров,КаталогВыгрузки,СледующаяВерсия)
...
мРежимСохраненияАрхивов = Ложь;мНомерОчереди = 0;