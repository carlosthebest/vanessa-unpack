	...
	Пока СледующаяВерсия <= МаксимальнаяВерсияДляРазбора Цикл
		...
		Если СтрокаВерсии <> Неопределено Тогда
			...
			Попытка
				...
				успешно = ВыгрузитьФайлыИзАрхива(СтруктураПараметров,СледующаяВерсия);
...
Функция СохранитьВыгруженныеФайлыВАрхив(СтруктураПараметров) Экспорт
	...
	Пока СледующаяВерсия <= МаксимальнаяВерсияДляРазбора Цикл
		...
		Если СтрокаВерсии <> Неопределено Тогда
			...
			ФайлАрхиваСоздан = ПроверитьСуществованиеФайлаКаталога(СтруктураПараметров.ПутьСохраненияАрхивов +  "\" + (СледующаяВерсия) + ".zip");
			ФайлБлокировкиСоздан = ПроверитьСуществованиеФайлаКаталога(СтруктураПараметров.ПутьСохраненияАрхивов +  "\" + (СледующаяВерсия) + ".lock");
			...
			Если НЕ (ФайлАрхиваСоздан ИЛИ ФайлБлокировкиСоздан) Тогда 
				Попытка					
					...
					СоздатьУникальныйФайл(СтруктураПараметров.ПутьСохраненияАрхивов +  "\" + (СледующаяВерсия) + ".lock");
				...
				ДобавитьВыгруженныеФайлыВАрхив(СтруктураПараметров,КаталогВыгрузки,СледующаяВерсия);
				УдалитьФайлы(СтруктураПараметров.ПутьСохраненияАрхивов +  "\" + (СледующаяВерсия) + ".lock");
...
Функция ДобавитьВыгруженныеФайлыВАрхив(Знач СтруктураПараметров, Знач КаталогВыгрузки,Знач СледующаяВерсия)
	...
	СтрокаКоманды = ПодставитьПараметрыВСтроку("""%1"" a ""%2"" ""%3""  -xr!*.form", 
