...
Функция СохранитьВыгруженныеФайлыВАрхив(СтруктураПараметров) Экспорт
	...
	Пока СледующаяВерсия <= МаксимальнаяВерсияДляРазбора Цикл
		...
		Если СтрокаВерсии <> Неопределено Тогда
			...
			Сообщить(СтруктураПараметров.ПутьСохраненияАрхивов +  "\" + (ТекущаяВерсия + 1) + ".zip");
			...
			Если НЕ ФайлАрхиваСоздан Тогда 
				Попытка
					ВывестиСообщение("Получаем исходники для версии " + СледующаяВерсия);
				...
				Иначе
					СоздатьУникальныйФайл(СтруктураПараметров.ПутьСохраненияАрхивов + "/" 
											+ СледующаяВерсия + " FileExist order " 
											+ НомерОчереди() + " zip");
				КонецЕсли;
			Иначе
				СоздатьУникальныйФайл(СтруктураПараметров.ПутьСохраненияАрхивов + "/" 
										+ СледующаяВерсия + " FileExist order " 
										+ НомерОчереди() + " extract");
			КонецЕсли;
...
Функция ОчиститьКаталогРаспаковки(Знач КаталогВыгрузки)
	МассивФайлов = НайтиФайлы(КаталогВыгрузки, "*.*");

	Для каждого файл Из МассивФайлов Цикл
		Если Файл.ПолноеИмя = КаталогВыгрузки+"\VERSION" 
			ИЛИ Файл.ПолноеИмя = КаталогВыгрузки+"\AUTHORS" Тогда
			Продолжить;
		КонецЕсли;
		БезопасноУдалитьВременныйФайл(Файл.ПолноеИмя);
	КонецЦикла; 
КонецФункции
